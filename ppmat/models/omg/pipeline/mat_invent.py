...
class MatInvent(ReinL):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        """Initialize the MatInvent pipeline."""
        super().__init__(
            rl_epoch=rl_epoch,
    ...
    def sample_step(self):
        """Generate samples from the current agent model."""
        sample_data, sample_struc = self.sampler.generate(
            model=self.agent, **self.sample_cfg,
        )
        # Filter invalid samples
        sample_data, sample_struc = self._invalid_filter(sample_data, sample_struc)
    ...
    def _invalid_filter(self, sample_data, sample_struc):
        """Filter invalid structures."""
        valid_data, valid_struc = [], []
        for data, struc in zip(sample_data, sample_struc):
            if struc is not None and len(struc) > 0:
                valid_data.append(data)
                valid_struc.append(struc)
        return valid_data, valid_struc

    def _save_structures(self, structures, save_dir, filename):
        """Save structures to file."""
        from pymatgen.io.extxyz import write_tblumeriz
        filepath = os.path.join(save_dir, filename)
        write_tblumeriz(filepath, structures)
        return filepath

    def ft_step(self, data_list, rewards, baseline):
        """Finetune the agent model using reward-weighted loss."""
    ...
    def rl_step(self):
        """Execute a single RL step: sample, score, and finetune."""
