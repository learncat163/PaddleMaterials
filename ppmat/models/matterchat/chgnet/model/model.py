...
class CHGNet(nn.Layer):
    ...
    def _compute(
        ...
        for idx, (atom_layer, bond_layer, angle_layer) in enumerate(
            ...
            if idx == self.n_conv - 2:
                ...
                if site_wise:
                    ...
                    prediction["m"] = list(
                        paddle.split(magmom.reshape([-1]), atoms_per_graph.tolist())
                    )
        ...
        if self.is_intensive:
            energy = energy / atoms_per_graph.cast(energy.dtype)
        prediction["e"] = energy
    ...
    def predict_graph(
        ...
        model_device = next(self.parameters()).place
        if type(graph) == CrystalGraph:
            ...
            for key, pred in prediction.items():
                if key == "e":
                    ...
                    assert len(pred) == 1
                    out[key] = pred[0].detach().numpy()
                elif key == "crystal_fea":
                    out[key] = pred.reshape([-1]).detach().numpy()
            return out
        if type(graph) == list:
            self.eval()
            ...
            for n in range(n_steps):
                ...
                for key, pred in prediction.items():
                    if key in ["e"]:
                        for i, e in enumerate(pred.detach().numpy()):
                        ...
                        for i, atom_fea in enumerate(pred):
                            predictions[n * batch_size + i][key] = (
                                atom_fea.detach().numpy()
                            )
                    elif key == "crystal_fea":
                        for i, crystal_fea in enumerate(pred.detach().numpy()):
