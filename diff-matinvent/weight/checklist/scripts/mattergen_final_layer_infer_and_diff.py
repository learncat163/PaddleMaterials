...
流程：
  ...
  4. 对比最终输出 pred_lattice / pred_frac_coords / pred_atom_types
...
SHARED_INPUT_JSON  = OUTPUT_DIR / "mattergen_final_infer_and_diff_fixed_input.json"
PT_OUTPUT_JSON     = OUTPUT_DIR / "mattergen_final_pytorch_output.json"
DIFF_REPORT_OUT_JSON           = OUTPUT_DIR / "mattergen_final_infer_and_diff_report.json"
...
_PT_RUNNER_SCRIPT = Path(__file__).parent / "mattergen_final_layer_pt_runner.py"
...
def run_paddle_forward(
    ...
    """运行 Paddle 推理，返回 pred_lattice, pred_frac_coords, pred_atom_types"""
    # 构建 structure_array
    structure_array = {
...
def generate_markdown_report(
    ...
    lines += _cmp_section("对比 1：pred_lattice（晶格预测）", pl_cmp)
    lines += _cmp_section("对比 2：pred_frac_coords（坐标预测）", px_cmp)
    lines += _cmp_section("对比 3：pred_atom_types（原子类型预测）", pa_cmp)
    lines += [
        "---",
        "",
...
def main():
    logger.info("=" * 60)
    logger.info("MatterGen 最终层 infer+diff（固定输入,paddle+原版推理 并diff）")
    logger.info("=" * 60)
    ...
    pd_pl, pd_px, pd_pa = run_paddle_forward(pd_model, test_input)
    ...
    md = generate_markdown_report(
        test_input, pd_pl, pd_px, pd_pa, pt_pl, pt_px, pt_pa,
        pl_cmp, px_cmp, pa_cmp,
    )
