...
PyTorch MatterGen 推理子脚本 —— 在 matinvent conda 环境下执行。

用法（由 mattergen_first_layer_infer_and_diff.py 通过 subprocess 调用）：
  python mattergen_first_layer_pt_runner.py <RAW_ROOT> <INPUT_JSON> <PT_CKPT> <OUT_JSON>
...
handle = model.diffusion_module.model.gemnet.atom_emb.register_forward_hook(_hook)
...
print(f"[PT] atom_emb shape={fl.shape}, mean={fl.mean():.6f}", flush=True)
