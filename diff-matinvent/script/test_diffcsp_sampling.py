...
def create_sampling_batch(batch_size=16):
    """创建采样批次 - 添加随机原子类型作为条件输入"""
    num_atoms_list = []
    atom_types_list = []

    for i in range(batch_size):
        np.random.seed(RANDOM_SEED + i)
        num_atoms = np.random.randint(4, 24)
        num_atoms_list.append(num_atoms)

        # 生成随机原子类型 (1-94，元素周期表前94个元素)
        atom_types = np.random.randint(1, 95, size=num_atoms).tolist()
        atom_types_list.extend(atom_types)

    batch_data = {
        'structure_array': {
            'num_atoms': paddle.to_tensor(num_atoms_list, dtype='int64'),
            'atom_types': paddle.to_tensor(atom_types_list, dtype='int64'),
        }
    }
...
def analyze_sample(sample_data, logger=None):
    ...
    stats = {
        ...
        'properties': {
            ...
            'lattice_mean': float(lattice.mean().item()),
            'lattice_std': float(lattice.std().item()),
        }
    }
...
def test_sampling(model, batch_size=16, num_inference_steps=10, logger=None):
    """测试采样功能"""
    if logger:
        logger.info("=" * 60)
        logger.info("TEST: DiffCSP Sampling")
        logger.info("=" * 60)
    ...
    batch_data = create_sampling_batch(batch_size)
    ...
    try:
        # 采样 - 返回格式是 {"result": result}
        # result 是一个列表，每个元素是一个包含生成样本的字典
        ...
        # 处理返回的结果
        results_list = samples['result']

        for i, sample_result in enumerate(results_list):
            sample_data = {
...
def compare_with_pytorch(results, logger=None):
    ...
    # 读取PyTorch结果
    pytorch_file = PROJECT_ROOT / "raw-matinvent" / "diff_tmp" / "diffcsp" / "sampling_results.json"
...
def run_test(device="gpu:0", num_inference_steps=10):
    ...
    logger.info("=" * 60)
    logger.info("DIFFCSP SAMPLING TEST")
    logger.info("=" * 60)
    ...
    if results is None:
        logger.error("Sampling test failed")
        return
