...
def set_random_seed():
    ...
    # PaddlePaddle deterministic settings
...
def create_test_batch(batch_size: int = 4, device: str = "gpu:0", logger: logging.Logger = None):
    ...
    # Create test data structure compatible with PaddlePaddle
    # Simple cubic crystal structure with 8 atoms
    num_atoms = 8
    total_atoms = num_atoms * batch_size

    # Simple positions (fractional coordinates)
    pos_list = []
    ...
    # Expand cell for batch
    cell = np.tile(cell, (batch_size, 1, 1))

    # Atomic numbers (silicon)
    atomic_numbers = np.array([14] * total_atoms, dtype=np.int64)
    ...
    if logger:
        ...
        logger.info(f"  cell shape: {batch['cell'].shape}")
        logger.info(f"  atomic_numbers shape: {batch['atomic_numbers'].shape}")
...
def benchmark_forward_pass(
    ...
    for run_idx in range(num_runs):
        # Create timestep
        t_idx = run_idx % 1000  # Cycle through timesteps
        t = paddle.to_tensor([float(t_idx)] * len(batch['num_atoms']), dtype='float32')

        with paddle.no_grad():
            try:
                # Forward pass - sample method
                output = model.sample(batch, num_inference_steps=1)
                ...
                run_result = {
                    ...
                    'outputs': {
                        ...
                        'pos_max': float(batch['pos'].max().numpy()),
                        'pos_min': float(batch['pos'].min().numpy()),
                        'cell_mean': float(batch['cell'].mean().numpy()),
                        'cell_std': float(batch['cell'].std().numpy()),
                    }
                }
...
def benchmark_sampling(
        ...
        for batch_idx in range(num_batches):
            ...
            with paddle.no_grad():
                try:
                    ...
                    for i in range(batch_size):
                        start_idx = i * 8  # 8 atoms per sample
                        end_idx = start_idx + 8
                        ...
                        sample_result = {
                            ...
                            'num_atoms': 8,
                            'properties': {
                                'pos_mean': float(batch['pos'][start_idx:end_idx].mean().numpy()),
                                'pos_std': float(batch['pos'][start_idx:end_idx].std().numpy()),
                                'cell_mean': float(batch['cell'][i].mean().numpy()),
                                'cell_std': float(batch['cell'][i].std().numpy()),
                            }
                        }
                        results['samples'].append(sample_result)

                        if logger and (batch_idx * batch_size + i) < 5:
                            logger.info(f"Sample {sample_result['sample_id']}: n_atoms={sample_result['num_atoms']}")
    ...
    if results['samples']:
        pos_means = [s['properties']['pos_mean'] for s in results['samples']]
        cell_means = [s['properties']['cell_mean'] for s in results['samples']]

        results['statistics'] = {
            'pos_mean_mean': float(np.mean(pos_means)),
            'pos_mean_std': float(np.std(pos_means)),
            'cell_mean_mean': float(np.mean(cell_means)),
            'cell_mean_std': float(np.std(cell_means)),
        }
    ...
    if logger:
        ...
        if 'statistics' in results:
            logger.info(f"Statistics: pos_mean={results['statistics']['pos_mean_mean']:.2f}")
...
def run_full_benchmark(device: str = "gpu:0"):
    ...
    summary = {
        ...
        'sampling': {
            'num_samples': len(sampling_results.get('samples', [])),
        },
    }
