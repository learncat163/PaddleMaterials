...
class TestBasicFunctionality(unittest.TestCase):
    ...
    def test_mattergen_has_rl_methods(self):
        """Test MatterGen RL adapter provides RL methods."""
        ...
        # Use default configuration for MatterGen
        # (from mattergen_suite.py _MATTERGEN_DEFAULT_CFG)
        decoder_cfg = {
            'gemnet_cfg': {
                'num_targets': 1,
                'latent_dim': 512,
        ...
        lattice_noise_scheduler_cfg = {'__class_name__': 'LatticeVPSDEScheduler', '__init_params__': {}}
        coord_noise_scheduler_cfg = {'__class_name__': 'NumAtomsVarianceAdjustedWrappedVESDE', '__init_params__': {}}
        ...
        try:
            ...
            # If MatterGen initialization fails, we can still test the adapter class
            # Just verify the adapter has the required methods
            self.assertTrue(hasattr(MatterGenRLAdapter, "add_noise"))
            self.assertTrue(hasattr(MatterGenRLAdapter, "calc_sample_loss"))
            self.assertTrue(hasattr(MatterGenRLAdapter, "calc_kl_reg"))
            return

        # Wrap with adapter
        adapted_model = create_matinvent_adapter(base_model)

        # Check that RL methods exist on the adapter
        self.assertTrue(hasattr(adapted_model, "add_noise"))
        self.assertTrue(hasattr(adapted_model, "calc_sample_loss"))
        self.assertTrue(hasattr(adapted_model, "calc_kl_reg"))

        # Verify the adapter can delegate to base model
        self.assertTrue(hasattr(adapted_model, "forward"))
        self.assertTrue(hasattr(adapted_model, "sample"))
